...
public abstract class DataSourceJPA<T, PK> implements InterfaceDataSourceJPA<T, PK> {
    ...
    @Override
    public void deleteBySomething(String query, Object value) {
        try {
            em.getTransaction().begin();
            em.remove(em.createQuery("from " + entityClass.getName() + " " + query, entityClass).setParameter(1, value).getSingleResult());
            em.getTransaction().commit();
    ...
    @Override
    public void deleteByMore(String query, int nPar, Object[] values) {
        try {
            em.getTransaction().begin();
